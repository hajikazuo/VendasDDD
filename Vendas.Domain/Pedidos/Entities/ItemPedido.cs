...
namespace Vendas.Domain.Pedidos
{
    public sealed class ItemPedido : Entity
    {
        ...
        public void AplicarDesconto(decimal desconto)
        {
            ...
            Guard.Against<DomainException>(desconto > PrecoUnitario * Quantidade, "Desconto não pode exceder o valor do item.");
        ...
        public void RemoverUnidades(int unidades)
        {
            ...
            Guard.Against<DomainException>(unidades > Quantidade, "Não é possivel remover mais unidades do que existem no item.");

            Quantidade -= unidades;

            Guard.Against<DomainException>(Quantidade == 0, "Um item de pedido não pode ter quantidade zero. Use o método da classe pedido para removê-lo.");
